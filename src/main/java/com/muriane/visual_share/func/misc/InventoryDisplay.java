...
public class InventoryDisplay {
    ...
    public static final Identifier INVENTORY_DISPLAY = Identifier.fromNamespaceAndPath(VisualShare.MOD_ID, "inventory_display");

    @SubscribeEvent
    public static void onKey(InputEvent.Key event){
        Minecraft mc = Minecraft.getInstance();
        if (event.getAction() == InputConstants.PRESS) {
            if (event.getKey() == ModKeys.MISC_INVENTORY_DISPLAY.getKey().getValue()) {
                if (Config.SERVER.ENABLE_INVENTORY_DISPLAY.get()){
                    if (mc.level != null && mc.player != null) { // 用level来判断玩家是否已经在某个服务器中
                        String marker = Config.SERVER.INVENTORY_DISPLAY_MARKER.get();
                    ...
                    if (mc.player != null){
                        mc.player.sendOverlayMessage(Component.translatable("overlay.visual_share.misc.inventory_display.not_enable"));
                    }
                }
            }
        }
    ...
    public static MutableComponent tranInfoToInventory(Component chat, Player player){
        ...
        if (!marker.isEmpty()){
            ...
            for (Component component : chat.toFlatList()) {
                ...
                while (!cur.equals(Component.empty()) && !end && time > 0) {
                    ...
                    if (i != -1) {
                        ...
                        newChat.append(Component.translatable("chat.visual_share.inventory_display", player.getName()).withStyle(style -> style
            ...
            LOGGER.warn("Inventory display marker is empty");
